...
const FormatBuilder = require('./format').builder;
const removeEmptyValues = require('../utils').removeEmptyValues;

var Banner = function(w, h, wmax, hmax, wmin, hmin, id, btype, battr, pos, mimes, topframe, expdir, api, ext, format){
  this.w = w;
  this.h = h;
  this.wmax = wmax;
  this.hmax = hmax;
...
BannerBuilder.prototype.format = function(format){
  const builder = new FormatBuilder();
  if (format) {
    this._format = format.map((format) => {
      builder
      .w(format.w)
      .h(format.h)
      .ext(format.ext);
